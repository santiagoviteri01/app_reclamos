...
def cargar_datos(uploaded_file_reclamos, uploaded_file_asegurados):
    try:
        ...
        pagados = pd.read_excel(uploaded_file_reclamos, sheet_name="PAGADOS20242025")
        pendientes = pd.read_excel(uploaded_file_reclamos, sheet_name="PENDIENTE20242025")
        
        # Cargar datos de asegurados
        asegurados = pd.read_excel(uploaded_file_asegurados)
        
        return pagados, pendientes, asegurados
    except Exception as e:
        st.error(f"Error al cargar los datos: {e}")
        return None, None, None

# Carga de datos
uploaded_file_reclamos = st.sidebar.file_uploader("Sube archivo de RECLAMOS", type=["xlsx"])
uploaded_file_asegurados = st.sidebar.file_uploader("Sube archivo de ASEGURADOS", type=["xlsx"])

if uploaded_file_reclamos and uploaded_file_asegurados:
    pagados, pendientes, asegurados = cargar_datos(uploaded_file_reclamos, uploaded_file_asegurados)
